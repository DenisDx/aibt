...
def run_memoryd_cron(root_dir: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    log("memoryd", "info", "call memoryd.cron.run_memoryd_cron")
    ...
    if not memoryd_cfg.get("enabled", False):
        ...
        try:
            ...
            return {"enabled": False, "queue": {"picked": 0, "started": 0, "done": 0, "failed": 0, "pruned": 0, "skipped": 0}}
        if pending <= 0:
    ...
    return {"enabled": True, "queue": result}
